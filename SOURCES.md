# Sources and design basis

Agent Skill Kit contains original, concise workflow guidance written for this repository. Some design patterns were informed by public skill collections and official documentation; this file records those sources so contributors can re-check version-sensitive details.

The project does not redistribute private account Knowledge, user preferences, repository-specific instructions, credentials, or internal research artifacts. Contributors remain responsible for checking the license and attribution requirements of any external text, code, examples, or assets they add.

## Skill and workflow sources

- [Vercel agent skills](https://github.com/vercel-labs/agent-skills) — frontend, browser, and agent workflow patterns.
- [obra/superpowers](https://github.com/obra/superpowers) — planning, debugging, testing, and software-workflow patterns.
- [VoltAgent awesome-agent-skills](https://github.com/VoltAgent/awesome-agent-skills) — discovery index for agent-skill ecosystems.
- [Google skills](https://github.com/google/skills) — official examples of technology-specific skill packaging.
- [K-Dense scientific agent skills](https://github.com/k-dense-ai/scientific-agent-skills) — research, reproducibility, and scientific workflow patterns.
- [CloudAI-X Three.js skills](https://github.com/cloudai-x/threejs-skills) — browser 3D workflow patterns.
- [Netresearch Docker development skill](https://github.com/netresearch/docker-development-skill) — container-development patterns.

## Official technical references

- [OWASP Application Security Verification Standard](https://owasp.org/www-project-application-security-verification-standard/)
- [OWASP Web Security Testing Guide](https://owasp.org/www-project-web-security-testing-guide/)
- [NIST Secure Software Development Framework](https://csrc.nist.gov/projects/ssdf)
- [MDN Web Docs](https://developer.mozilla.org/)
- [React documentation](https://react.dev/learn)
- [TypeScript Handbook](https://www.typescriptlang.org/docs/handbook/intro.html)
- [Tailwind CSS documentation](https://tailwindcss.com/docs)
- [Node.js documentation](https://nodejs.org/docs/latest/api/)
- [C2PA Technical Specification](https://spec.c2pa.org/specifications/specifications/2.4/specs/C2PA_Specification.html)
- [ExifTool](https://exiftool.org/)
- [Creative Commons FAQs](https://creativecommons.org/faq/)
- [Google AdSense policies](https://support.google.com/adsense/answer/48182?hl=en)
- [Google AdMob policies](https://support.google.com/admob/answer/6128543?hl=en)
- [Stripe subscriptions and webhooks](https://docs.stripe.com/billing/subscriptions/webhooks)
- [Apple In-App Purchase](https://developer.apple.com/in-app-purchase/)
- [Google Play subscriptions](https://developer.android.com/google/play/billing/subscriptions)
- [FTC Endorsement Guides](https://www.ftc.gov/business-guidance/resources/ftcs-endorsement-guides-what-people-are-asking)

The defensive-security skills use OWASP ASVS for control expectations, OWASP WSTG for scoped web-testing scenarios, and NIST SSDF for secure-development and supply-chain practices. These are reference frameworks, not permission to test any target; users must provide authorization and scope separately.

- [MIT OpenCourseWare Differential Equations](https://ocw.mit.edu/courses/18-03sc-differential-equations-fall-2011/) — open course structure for modeling, solving, and interpreting differential equations.
- [MIT OpenCourseWare Numerical Methods for Partial Differential Equations](https://ocw.mit.edu/courses/18-336-numerical-methods-for-partial-differential-equations-spring-2009/) — numerical-simulation concepts, methods, and stability-oriented study material.
- [SymPy ODE Documentation](https://docs.sympy.org/latest/modules/solvers/ode.html) — versioned documentation for classifying, solving, and checking supported ordinary differential equations.
- [NIST SP 800-61 Rev. 3](https://csrc.nist.gov/pubs/sp/800/61/r3/final) — current official incident-response recommendations and considerations for cybersecurity risk management.

## Version-sensitive guidance

Sources, platform requirements, package APIs, security recommendations, and legal or advertising policies can change. Re-check the authoritative source before relying on a skill for a production, financial, privacy, security, monetization, or platform-policy decision.


## Mathematics and education references

- [OpenStax Mathematics](https://openstax.org/subjects/math) — open textbooks and resources covering algebra, trigonometry, precalculus, calculus, statistics, and data science.
- [MIT OpenCourseWare Single Variable Calculus](https://ocw.mit.edu/courses/18-01sc-single-variable-calculus-fall-2010/) — independent-study calculus materials with lectures, worked examples, problems, and solutions.
- [MIT OpenCourseWare Multivariable Calculus](https://ocw.mit.edu/courses/18-02sc-multivariable-calculus-fall-2010/) — multivariable and vector-calculus course structure and problem-solving materials.
- [NIST Digital Library of Mathematical Functions](https://dlmf.nist.gov/) — authoritative reference for special functions, notation, numerical methods, and mathematical computation.

These sources inform original workflows only. The repository does not copy textbook passages, answer keys, or proprietary course content.

## Science, technology, and applied-computation references

- [OpenStax Science](https://openstax.org/subjects/science) — open, peer-reviewed science textbooks covering biology, chemistry, physics, and related subjects.
- [NIST SI Units](https://www.nist.gov/pml/owm/metric-si/si-units) — official guidance on SI base and derived units, prefixes, notation, and dimensional communication.
- [scikit-learn Model Evaluation](https://scikit-learn.org/stable/modules/model_evaluation.html) — versioned documentation on selecting scoring functions, cross-validation, metrics, and decision-aware evaluation.
- [SymPy Documentation](https://docs.sympy.org/latest/index.html) — versioned documentation for symbolic mathematics, solvers, calculus, matrices, logic, and numerical evaluation.
- [Mermaid Documentation](https://mermaid.js.org/intro/) — official documentation for text-defined diagrams and supported diagram types.
- [Google SRE Workbook: Implementing SLOs](https://sre.google/workbook/implementing-slos/) — public guidance on service-level indicators, objectives, error budgets, and reliability decisions.
- [NIST AI Risk Management Framework](https://www.nist.gov/itl/ai-risk-management-framework) — official voluntary framework for managing AI risks and trustworthy evaluation.
- [W3C Web Content Accessibility Guidelines Overview](https://www.w3.org/WAI/standards-guidelines/wcag/) — official accessibility standards overview and version guidance.
- [MedlinePlus: Evaluating Health Information](https://medlineplus.gov/evaluatinghealthinformation.html) — official health-information source evaluation and professional-care boundary guidance.

These references inform original workflow design only. The repository does not copy source text, proprietary templates, or vendor-specific private instructions.
